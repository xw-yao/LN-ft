    ...
    def preprocess_dataset(self, padding, max_sequence_len, batch_size, sample_size=None, random_seed=42):
        ...
        class glue_evaluator:
            ...
            def preprocess_dataset(self, padding, max_sequence_len, batch_size, sample_size=None, random_seed=42):
                ...
                if sample_size is not None:
                    # make random indices and take a sample from the train dataset
                    subset_indices = np.random.RandomState(random_seed).permutation(len(datasets['train']))[
                                     :sample_size]
                    datasets['train'] = datasets['train'].select(subset_indices)
                ...
                if not is_regression:
                    label_list = datasets['train'].features['label'].names
                    self.idx_to_label = {k: v for k, v in
                                         enumerate(datasets['train'].features['label'].__dict__['_int2str'])}
                    self.num_labels = len(label_list)
                ...
                def _generate_train_prompts(examples):
                    prompted_example = (TASK_TO_PROMPT_TRAIN[self.task_name].format(sentence1=examples[sentence1_key],
                                                                                    label=NUM_TO_TEXT[self.task_name][
                                                                                        examples[
                    ...
                    result = self.tokenizer(prompted_example, padding=padding, max_length=max_sequence_len,
                                            truncation=True)
                    return result
                ...
                def _generate_eval_prompts_stsb(examples):
                    prompted_example = (TASK_TO_PROMPT_EVAL[self.task_name].format(sentence1=examples[sentence1_key],
                                                                                   sentence2=examples[sentence2_key]))
                    result = self.tokenizer(prompted_example, padding=padding, max_length=max_sequence_len,
                                            truncation=True)
                    return result
                ...
                if 'opt' in self.model_name:
                    if self.task_name == 'stsb':
                        datasets['train'] = datasets['train'].map(_generate_train_prompts_stsb,
                                                                  load_from_cache_file=False)
                        datasets['validation'] = datasets['validation'].map(_generate_eval_prompts_stsb,
                                                                            load_from_cache_file=False)
                        datasets['test'] = datasets['test'].map(_generate_eval_prompts_stsb, load_from_cache_file=False)
                        ...
                        datasets['validation_matched'] = datasets['validation_matched'].map(_generate_eval_prompts,
                                                                                            load_from_cache_file=False)
                        datasets['validation_mismatched'] = datasets['validation_mismatched'].map(
                            _generate_eval_prompts,
                            load_from_cache_file=False)
                        ...
                        datasets['train'] = datasets['train'].map(_generate_train_prompts, load_from_cache_file=False)
                        datasets['validation'] = datasets['validation'].map(_generate_eval_prompts,
                                                                            load_from_cache_file=False)
            ...
            def training_preparation(self, learning_rate, encoder_trainable, weight_decay, lora_alpha=None, lora_r=None,
                ...
                if 'opt' in self.model_name:
                    ...
                    if apply_lora:
                        config = BertConfig.from_pretrained(self.model_name, num_labels=self.num_labels,
                                                            apply_lora=True, lora_alpha=lora_alpha, lora_r=lora_r,
                                                            return_dict=True)
                    else:
                        config = BertConfig.from_pretrained(self.model_name, num_labels=self.num_labels,
                                                            return_dict=True)
            ...
            def train_and_evaluate(self, num_epochs, gradient_accumulation_steps, warmup_ratio, ft_type=None):
                ...
                if not self.data_loaders:
                    raise Exception(
                        'data loaders were not initialized, please run "preprocess_dataset" before training.')
                    ...
                    for step, batch in enumerate(progress_bar):
                        ...
                        if 'opt' in self.model_name:
                            ...
                            _predictions = _predictions[:,
                                           :-1]  # double check that _predictions do not end on a token that is a part of the label
                            _targets = targets[:, 1:]
